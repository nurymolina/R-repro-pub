...
knitr::opts_knit$set(root.dir = rprojroot::find_rstudio_root_file()) #direct where root project file is, knitr:optsknit is how we set global options, root directory is the project file

#set global options
knitr::opts_chunk$set(results=FALSE, eval = TRUE, echo= FALSE, warning= FALSE, message=FALSE) #these options willbe applied in all code chunks, you can override it though if you need to
```
...
![Fig. 1 provides an exact overview of the experimental timeline for each measure we collected. All experimental sessions took place from 3 p.m. to 6 p.m. to control for circadian variations of hormonal levels. Participants were assigned to the two experimental conditions pseudo-randomly.](fig/Fig1.jpg)
...
```{r fig3-heartrate, results=FALSE, eval = TRUE, echo= FALSE, warning= FALSE, message=FALSE, fig.cap= "Fig 3: Mean heart rate of stress and control groups at baseline and during intervention."}
library(tidyverse)
library(BayesFactor)
...
```{r}
#run our code from code directory
source("code/02_hormone_analysis.R", local=knitr::knit_global()) #second arg makes sure its sourcing it from the right place
